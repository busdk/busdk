...
- `bus reports` handles `prior-year-correction` / `opening-adjustment` source kinds only partially: `FR-REP-010` excludes those rows from the balance-sheet equity-change side, but `profit-and-loss` still includes their P&L-account effects in the current-period result.
  - Repro:
    - In [2024-01-01.bus](/Users/jhh/git/sendanor/sendanor-books-2023/exports/sendanor/2024/2024-01-01.bus), post `2024-01-01` memoranda tagged with:
      - `--source-id FY2023FIX1 --source-kind prior-year-correction`
      - `--source-id FY2023FIX2 --source-kind prior-year-correction`
      - `--source-id FY2023FIX3 --source-kind prior-year-correction`
      - `--source-id FY2023FIX4 --source-kind prior-year-correction`
    - The correction rows include prior-year carry repair hitting:
      - `7611 / 2880` for `204.62`
      - `7611 / 2870` for `446.39`
      - `1920 / 1940 / 1999 / 8564` for net `54.49`
    - Then run:
      - `make -C exports/sendanor/2024 export`
      - `make -C exports/sendanor/2024 reports`
  - Current behavior:
    - The old huge mismatch caused by ordinary year-start correction memos is gone.
    - But `evidence-pack` still fails `FR-REP-010` with:
      - `period result from profit-and-loss (13235.77) does not equal balance-sheet equity change ... delta: 13832.29`
    - The remaining gap is exactly `596.52`, which equals:
      - `204.62 + 446.39 - 54.49`
    - That is the net P&L impact of the tagged prior-year-correction rows (`7611`, `8564`), which shows:
      - the balance-sheet/reconciliation side is excluding those corrections
      - but `profit-and-loss` still includes them in the current-period result
  - Expected:
    - when rows are explicitly tagged as `prior-year-correction` or `opening-adjustment`, both sides of the income-result reconciliation should treat them consistently as non-operating opening-state repair work
    - either:
      - exclude their P&L-account contributions from current-period `profit-and-loss`, or
      - include them consistently on both sides of the same reconciliation path
    - the intended semantic model from the new source-kind support is that these rows stay visible in journal/balance-sheet audit output without distorting current-period operating result
  - Repo impact:
    - Sendanor 2024 can now tag year-start correction memoranda semantically, but statutory `tuloslaskelma*` generation still fails because the new classification is only half-applied
