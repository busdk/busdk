...
IMAGE=${BUS_REMOTE_WORKER_BUILD_IMAGE:-bus-integration-task:local-image-smoke}
PLATFORM=${BUS_REMOTE_WORKER_BUILD_PLATFORM:-linux/amd64}
GO_VERSION=${BUS_REMOTE_WORKER_BUILD_GO_VERSION:-1.26.5}
SUBMODULE_MODE=${BUS_REMOTE_WORKER_BUILD_SUBMODULE_MODE:-pinned}
SUBMODULES=${BUS_REMOTE_WORKER_BUILD_SUBMODULES:-"bus bus-agent bus-api-provider-auth bus-api-provider-events bus-dev bus-events bus-help bus-integration bus-integration-task bus-integration-docker bus-integration-containers bus-lint bus-notes bus-operator-token bus-preferences bus-remote bus-secrets bus-update logs"}
