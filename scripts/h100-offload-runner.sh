...
remote_preflight_script() {
	...
	printf '%s' "set -eu; cd $root_q; dirty=\$(git status --porcelain); if [ -n \"\$dirty\" ]; then printf 'dirty remote checkout at %s\n' $root_q >&2; git status --short | sed -n '1,80p' >&2; exit 20; fi; command -v docker >/dev/null 2>&1 || { echo 'missing docker' >&2; exit 21; }; docker image inspect $image_q >/dev/null 2>&1 || { echo 'missing worker image: '$image_q >&2; exit 22; }; command -v curl >/dev/null 2>&1 || { echo 'missing curl' >&2; exit 23; }; curl -fsS --max-time $timeout_q $events_q/api/v1/events/capabilities >/dev/null || { echo 'missing Events capabilities at '$events_q >&2; exit 24; }; curl -fsS --max-time $timeout_q http://127.0.0.1:11434/api/tags | grep -F $model_pattern_q >/dev/null || { echo 'missing Ollama model: '$model_q >&2; exit 25; }; printf 'preflight ok: root=%s image=%s model=%s events=%s\n' $root_q $image_q $model_q $events_q"
}
