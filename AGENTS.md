...
79. Current AI Platform API planning: `/v1/*` remains OpenAI-compatible and SDK-facing; the `bus-auth` client CLI supplies tokens for those calls. `bus-status` owns the client CLI for VM lifecycle status at `/api/v1/vm/status`. `bus-containers` owns the client CLI for end-user container runner APIs at `/api/v1/containers/status`, `/api/v1/containers/runs`, and `/api/v1/containers/runner`. `bus-api-provider-auth` owns the auth service implementation, while `bus-auth` owns the auth client CLI. `/api/internal/usage-events` is internal api-proxy/billing infrastructure and should not get a Bus CLI module unless explicitly requested.
