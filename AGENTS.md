...
12.3.5. Dev-task Codex worker images must expose the current mounted BusDK tool set through script-friendly command wrappers that preserve the caller working directory. At minimum `bus`, `bus-dev`, `bus-lint`, `bus-gx`, and `bus-notes` must be available on `PATH`, so worker tasks can run `bus lint`, `bus gx`, and note/reporting commands without host-only installs.
