...
39. When an e2e suite depends on an external environment capability (for example loopback TCP bind), probe that capability once in the suite runner and emit explicit `SKIP <test>: <reason>` lines for affected cases instead of failing them generically when the environment forbids the capability.
