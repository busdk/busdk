...
4. Use the documented local development system as the default execution path for broad module work: issue parallel module work through `bus dev work` / `bus dev task` when the task system is available, and prefer controller-owned worker provisioning over manual container/script starts.
