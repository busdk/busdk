...
tmp_dir="$root_dir/tmp/worktrees/sync-submodules-configured-branch-repair-test.$$"
...
git_config -C "$tmp_dir/root-checkout/module-a" fetch --quiet origin develop
git_config -C "$tmp_dir/root-checkout/module-a" checkout --quiet -B develop origin/develop
git_config -C "$tmp_dir/root-checkout/module-a" branch --unset-upstream develop >/dev/null

sync_out="$tmp_dir/sync-no-upstream.out"
...
test "$(git -C "$tmp_dir/root-checkout/module-a" rev-parse --abbrev-ref --symbolic-full-name '@{u}')" = "origin/develop"

git_config -C "$tmp_dir/root-checkout/module-a" checkout --quiet -b supervisor/diagnostic
test "$(git -C "$tmp_dir/root-checkout/module-a" rev-parse HEAD)" = "$sub_head"

sync_out="$tmp_dir/sync-wrong-branch.out"
if ! GIT_ALLOW_PROTOCOL=file "$tmp_dir/root-checkout/scripts/sync-submodules.sh" --pull-only --jobs 1 --verbose module-a >"$sync_out"; then
	cat "$sync_out" >&2
	exit 1
fi
...
printf 'sync submodules configured branch repair OK\n'
