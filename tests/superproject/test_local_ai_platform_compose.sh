...
for route in '/v1/' '/api/v1/auth/' '/api/v1/events' '/api/v1/billing/' '/api/internal/usage-events' '/api/v1/vm/' '/api/v1/containers/' '/api/v1/notes' '/api/internal/stripe/webhook'; do
...
grep -q 'BUS_DEV_TASK_RECIPIENT: bus-dev' "$tmp_dir/compose.config"
grep -q 'notes.write' "$tmp_dir/compose.config"
grep -q 'busdk_notes,busdk_concrete,busdk_busdata' "$tmp_dir/compose.config"
