...
grep -q '"action_queue_file": "' "$tmp_dir/state/policy-cycle.json"
grep -q '"schema_version": "busdk.supervisor.action_plan/v1"' "$tmp_dir/state/action-plan.json"
grep -q '"action_queue_file": "' "$tmp_dir/state/action-plan.json"
...
grep -q '"schema_version": "busdk.supervisor.action_queue/v1"' "$tmp_dir/state-classify/action-queue.json"
grep -q '"work_ref": "busdk#1"' "$tmp_dir/state-classify/action-queue.json"
grep -q '"route": "review_pin_candidate"' "$tmp_dir/state-classify/action-queue.json"
