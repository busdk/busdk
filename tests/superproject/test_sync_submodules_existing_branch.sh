...
tmp_dir="$root_dir/tmp/worktrees/sync-submodules-existing-branch-test.$$"
...
git_config -C "$tmp_dir/root-checkout/module-a" checkout --quiet --detach origin/develop
test -z "$(git -C "$tmp_dir/root-checkout/module-a" branch --show-current)"
test "$(git -C "$tmp_dir/root-checkout/module-a" rev-parse develop)" = "$sub_head"
...
printf 'sync submodules existing branch OK\n'
