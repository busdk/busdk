...
### Add opt-in normalized text matching for `bus files assert cell`

Problem:
- Real report assertions in this repo often target human-facing `section` labels from statutory CSV outputs.
- Those labels can contain indentation spaces that are present for visual hierarchy, not because the operator wants whitespace-sensitive matching.
- This makes report assertions brittle when only leading/trailing or repeated internal spaces differ, even though the semantic section label is the same.

Concrete repo pain:
- `exports/sendanor/2023/Makefile`, `exports/sendanor/2024/Makefile`, and `exports/sendanor/2025/Makefile` use many `bus files assert cell ... --row-filter 'section=...'` checks.
- Small whitespace drift in report labels can break rebuilds even when the accounting data and visible report meaning are unchanged.
- Today the workaround is to keep adjusting exact string literals in Makefiles instead of expressing the semantic intent directly.

Requested capability:
- Add an opt-in normalized matching mode for `bus files assert cell`, for example one of:
  - `--trim-space`
  - `--normalize-space`
  - `--row-filter-normalize-space 'section=...'`
- In that mode Bus should compare text values after predictable normalization such as:
  - trim leading/trailing whitespace
  - optionally collapse repeated internal whitespace to one space

Important scope:
- This should be opt-in, not the only/default behavior.
- Default exact matching still matters in cases where whitespace is intentionally significant.
- The goal is to let report assertions choose semantic matching when the source field is a presentation label.

Why this matters:
- In this repo, visible report hierarchy is often encoded with indentation, but the accounting control itself is about the line identity, not the exact padding.
- A first-class normalized assert mode would reduce noisy rebuild failures and eliminate shell-side workarounds for whitespace-only report drift.
