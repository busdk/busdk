...
- extend the Bus Events ecosystem with a configurable `bus-api` REST-to-event bridge, request/reply orchestration, provider discovery, durable replay, explicit ack/retry policy, and additional broker backends after the current `bus-events` and `bus-api-provider-events` MVP has settled; cloud/vendor integrations such as UpCloud must be event-based integration workers that listen for domain events and emit result events, not `bus-api-provider-*` REST API modules
...
- refactor `/Users/jhh/git/heusalagroup/ai-platform/api-proxy` into Bus provider modules incrementally: first extract `bus-api-provider-usage` for `/api/internal/usage-events`, then split cloud-neutral VM/container/LLM REST providers from UpCloud-specific event workers, preserving `api-proxy` simplicity by keeping users, OTPs, waitlist state, and admin state in Bus auth and keeping UpCloud implementation details out of REST providers
