...
- align Bus-owned AI Platform API planning with the in-progress AI Platform docs: keep OpenAI-compatible model calls under `/v1/*` and supply tokens through the `bus-auth` client CLI; `bus-status` owns the client CLI for VM lifecycle status at `/api/v1/vm/status`; `bus-containers` owns the client CLI for end-user container runner APIs at `/api/v1/containers/status`, `/api/v1/containers/runs`, and `/api/v1/containers/runner`; `bus-api-provider-auth` owns the auth service implementation; `bus-auth` owns the auth client CLI; leave `/api/internal/usage-events` as api-proxy/internal infrastructure with no Bus CLI module for now; and keep final auth/admin service paths pending final API docs
